...
from run_togt_planner.RaceGenerator.GateShape import (
    BaseShape,
    SingleBall,
    TrianglePrisma,
    RectanglePrisma,
...
def quote_specific_keys(data: Union[dict, List[dict], str], keys_to_quote: List[str] = KEYS_TO_QUOTE):
...
def create_gate(
    ...
    return Gate(gate_shape=gate_type(**shape_kwargs), position=position, stationary=stationary, name=name)
